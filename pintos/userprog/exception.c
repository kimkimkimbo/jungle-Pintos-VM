...
{
    bool not_present; /* True: 존재하지 않는 페이지, false: 읽기 전용 페이지에
                         쓰기. */
    bool write; /* True: 접근이 쓰기였음, false: 접근이 읽기였음. */
    bool user; /* True: 사용자에 의한 접근, false: 커널에 의한 접근. */
    void *fault_addr; /* 폴트 주소. */

    /* 폴트 주소를 얻습니다. 이는 폴트를 발생시킨 접근의 대상이 된 가상
       주소입니다. 이것은 코드나 데이터를 가리킬 수 있습니다. 이것이 반드시
       폴트를 발생시킨 명령어의 주소는 아닙니다 (그것은 f->rip입니다). */
